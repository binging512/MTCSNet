...
def gen_weak_label_with_point_fb(img, point_dict, superpixel, grad):
    _, grad, _, _ = cv2.connectedComponentsWithStats(grad, connectivity=4)
    ...
    for inst_idx, inst_prop in inst_points_dict.items():
        ...
        grad_value = grad[bnd_point_y, bnd_point_x]
        if grad_value == 0:
            ...
            weak_label_boundary[grad==grad_value] = 1
            weak_label_valid[grad==grad_value] = 1
...
def gen_grad(img):
    """Generate the image boundaries with sobel and remove the small objects

    Args:
        img (np.ndarry): the input image

    Returns:
        np.ndarray: the boundaries in the images
    """
    ...
    _, dst = cv2.threshold(dst.astype(np.uint8), 0, 1, cv2.THRESH_OTSU)
    dst = morphology.erosion(dst, morphology.disk(1))
    num_labels, labels, stats, centroids  = cv2.connectedComponentsWithStats(dst, connectivity=4)
...
def preprocess_MoNuSeg(image_dir, anno_dir, output_dir):
    ...
    output_semantic_dir = os.path.join(output_dir, 'semantics_v2')
    output_grad_dir = os.path.join(output_dir, 'grads_v2')
    output_vis_dir = os.path.join(output_dir, 'vis')
    os.makedirs(output_gt_dir,exist_ok=True)
    os.makedirs(output_semantic_dir,exist_ok=True)
    os.makedirs(output_grad_dir, exist_ok=True)
    ...
    for i, image_name in enumerate(sorted(os.listdir(image_dir))):
        ...
        # get the grad maps
        grad_map = gen_grad(img)
        save_grad_path = os.path.join(output_grad_dir, image_name.replace('.tif','.png'))
        cv2.imwrite(save_grad_path, grad_map)
...
def gen_weak_n_full_labels(image_dir, semantic_dir, grad_dir, output_dir):
    print("Generating the weak and full supervision labels...")
    output_superpixel_dir = os.path.join(output_dir,'superpixels_v2')
    output_point_dir = os.path.join(output_dir,'points_v2')
    output_weak_dir = os.path.join(output_dir, 'labels_v2/weak')
    output_weak_heat_dir = os.path.join(output_dir, 'labels_v2/weak_heatmap')
    output_inst_dir = os.path.join(output_dir, 'labels_v2/weak_inst')
    output_full_dir = os.path.join(output_dir, 'labels_v2/full')
    output_full_heat_dir = os.path.join(output_dir, 'labels_v2/full_heatmap')
    ...
    for ii, image_name in enumerate(sorted(os.listdir(image_dir))):
        ...
        grad = cv2.imread(os.path.join(grad_dir, image_name.replace('.tif','.png')), flags=0)
        point_dict = gen_point_supervision(semantic)
        superpixel, sp_vis = gen_superpixel_adaptive(img, point_dict)
        weak_label, instance_dict, weak_label_valid = gen_weak_label_with_point_fb(img, point_dict, superpixel, grad)
...
if __name__=="__main__":
    ...
    semantic_dir = './data/MoNuSeg/train/semantics_v2'
    grad_dir = './data/MoNuSeg/train/grads_v2'
    output_dir = './data/MoNuSeg/train'
    # preprocess_MoNuSeg(image_dir, anno_dir, output_dir)
    # gen_weak_n_full_labels(image_dir, semantic_dir, grad_dir, output_dir)
