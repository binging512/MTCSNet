...
def gen_point_supervision(semantic):
    ...
    background = morphology.dilation(background, morphology.disk(3))
    background_points= []
...
def gen_weak_n_full_labels(image_dir, semantic_dir, output_dir):
    ...
    output_weak_dir = os.path.join(output_dir, 'labels_v5/weak')
    output_weak_heat_dir = os.path.join(output_dir, 'labels_v5/weak_heatmap')
    output_inst_dir = os.path.join(output_dir, 'labels_v5/weak_inst')
    output_full_dir = os.path.join(output_dir, 'labels_v5/full')
    output_full_heat_dir = os.path.join(output_dir, 'labels_v5/full_heatmap')
    # output_degree_dir = os.path.join(output_dir, 'labels_v5/degree')
    ...
    # os.makedirs(output_degree_dir, exist_ok=True)
    for ii, image_name in enumerate(sorted(os.listdir(image_dir))):
        ...
        point_dict = json.load(open(os.path.join(output_point_dir,image_name.replace('.tif', '.json')), 'r'))
        # point_dict = gen_point_supervision(semantic)
        superpixel = tif.imread(os.path.join(output_superpixel_dir, image_name))
        # superpixel, sp_vis = gen_superpixel_adaptive(img, point_dict)
        vor_img = cv2.imread(os.path.join(output_voronoi_dir, image_name.replace('.tif','.png')), flags=0)
        ...
        # deg_dict = gen_degree_n_distance(semantic, point_dict)
        ...
        # pickle.dump(deg_dict, open(os.path.join(output_degree_dir, image_name.replace('.tif','.pkl')), 'wb'))
